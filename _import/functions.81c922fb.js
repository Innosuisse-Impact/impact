...
import { createColorScale, getBrandColors, getWaffleColorSet} from "./colors.6e42c185.js";
...
const language = {value: "de", label: "Deutsch"}
...
export function draw_fin_plot_legend() {
  return Plot.legend({ color: color_inst, columns: 1, swatchSize: 12, style: { fontSize: "12px", fontFamily: "sans-serif", fontWeight: 200, color: black_innosuisse } })
}
...
export function draw_dn(
  ...
  const df = df_innovationsart.filter(
    (d) => d.plot === plot && instrument.includes(d.instrument)
  );

  const df_highlight =
    plot === "digital"
      ? df.filter((d) => d.type === "Digitalisierung")
      : df.filter((d) => d.type === "Nachhaltigkeit");
  ...
  return Plot.plot({
    ...
    fx: {
      ...
      axis: "top",
      reverse: true,
      domain: plot !== "digital" ? ["soziale Nachhaltigkeit", "ökologische Nachhaltigkeit", "Nachhaltigkeit"]: undefined
    },
    fy: { label: null },
    style: {fontSize: "12px", fontFamily: "sans-serif", fontWeight: 200, color: black_innosuisse},
    x: x_axis_d,
    ...
    marks: [
      ...
      Plot.textX(
        df_highlight.filter(
          (d) => d.type === "Nachhaltigkeit" || "Digitalisierung"
        ),
        {
          x: 100,
          y: "respondant",
...
export function drawMiniPlot(instrument, funding = true) {
  ...
  return Plot.plot({
    ...
    marginRight: 30,
    marginLeft: 0,
    y: funding ? { label: "Mio. Fr." } : { label: label, domain: instrument !== "Start-up Coaching" ? undefined : [0,400] },
    x: {
...
export function leverage() {return Plot.plot({
    marginLeft: 0,
    marginRight: 0,
    ...
    marks: [
      ...
      Plot.text(
        [
          { x: 110, y: 105, text: "1 Fr. Förderung" },
          { x: 350, y: 105, text: "4 Fr. Wertschöpfung" }
        ],
        {
          x: "x",
          y: "y",
...
export function kof_did_plot() {return Plot.plot({
  marginLeft: 90,
  marginBottom: 45,
  marginRight: 158,
  marginTop: 45,
  x: {
    ...
    label: "Jahr in Bezug auf Abschluss des Innovationsprojekts"
  },
  y: {
    domain: [-75, 75],
    ...
    labelOffset: 15,
    label:
      "Unterschied zwischen Unternehmen mit Innosuisse-Förderung und der Kontrollgruppe in %"
  },
  fy: {
    label: null,
  ...
  marks: [
      ...
      [
        {
          x: "t+4",
          y: 0.14,
          text: "↑ Innosuisse-Unternehmen wachsen stärker"
        },
        {
          x: "t+4",
          y: -0.15,
          text: "↓ Kontrollgruppe-Unternehmen wachsen stärker"
        }
      ],
      {
        x: "x",
    ...
    Plot.line(
      [
        { type_de: "Umsatz", year_text: "t", y: 0.21 },
        { type_de: "Umsatz", year_text: "t+4", y: 0.21 },
        { type_de: "Beschäftigung", year_text: "t", y: 0.18 },
        { type_de: "Beschäftigung", year_text: "t+4", y: 0.18 }
      ],
      {
        x: "year_text",
        y: "y",
...
const df_toipis = [
    {
      type: "Innovationsschecks",
      pct: 36,
      instrument: "Starthilfe für Projekte und Vernetzung"
    },
    {
      type: "Innovationsprojekte",
      pct: 26,
      instrument: "Starthilfe für Projekte und Vernetzung"
    }
];

export function ib_toipis() {return Plot.plot({
    marginTop: 45,
  marginLeft: 150,
  width: 320,
    color: color_inst,
...
export function su_vza() {return Plot.plot({
    marginTop: 45,
    marginBottom: 35,
    height: 120,
    marginLeft: 5,
    ...
    x: {
      ...
      label:
        "⌀ Anzahl Vollzeitbeschäftigte nach Abschluss des Core Coachings und 3 Jahre danach"
    },
    y: { axis: null },
    marks: [
      Plot.barX(
        ...
        [
          {
            type: "Vollzeitbeschäftigte",
            pct: 18.5,
            text: "+11.7 Vollzeitbeschäftigte",
            instrument: "Begleitung von Start-ups"
          }
        ],
        {
...
export function su_vza_legend() {return Plot.legend({ color: color_su, style:{fontSize: "12px", fontFamily: "sans-serif", fontWeight: 200, color: black_innosuisse}})}